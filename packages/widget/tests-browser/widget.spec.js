...
test("answers a question, and renders the answer", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Paying" })).toBeVisible({
    timeout: 20000,
  });
...
test("sends the question to Onyx with the persona and tool pinned", async ({
  page,
}) => {
  await stubOnyx(page);
  const sent = [];
  page.on("request", (request) => {
    if (request.url().includes(ONYX))
...
test("shows the sources it answered from", async ({ page }) => {
  ...
  await expect(page.getByText("Direct Debit")).toBeVisible({ timeout: 20000 });
  await expect(
    page.getByRole("link", { name: /Paying your Council Tax/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: /Council Tax bands/ }),
  ).toBeVisible();
  expect(errors).toEqual([]);
});
...
test.describe("against the real Onyx", () => {
  ...
  test("answers a real question with real sources", async ({ page }) => {
    ...
    const input = page.locator('textarea[placeholder="Ask me anything…"]');
    await input.fill(
      process.env.LIVE_QUESTION || "How do I pay my council tax?",
    );
    await input.press("Enter");
...
test.describe("on a phone", () => {
  ...
  test("the input stays reachable with an answer on screen", async ({
    page,
  }) => {
